    ...
    <section id="experience" className="relative py-32 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative">
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-px timeline-rail" />
                ...
                >
                  <div className={`hidden md:block flex-1 ${i % 2 === 0 ? 'text-right' : 'text-left'}`}>
                    {i % 2 === 0 ? (
                      <div className="glass-card rounded-2xl p-8 gradient-border">
                        ...
                        <p className="text-[#3e7bfa] font-medium mb-1">{exp.company}</p>
                        <p className="text-[#5b6478] text-sm mb-4">{exp.description}</p>
                        <div className="flex flex-wrap gap-2 justify-end">
                          {exp.achievements.slice(0, 2).map((a) => (
                            <span key={a} className="text-xs text-[#93a0b8] bg-[rgba(255,255,255,0.03)] px-3 py-1 rounded-full border border-[rgba(255,255,255,0.06)]">
                              {a}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="absolute left-8 md:left-1/2 -translate-x-1/2 z-10">
                    <motion.div
                      ...
                      className={`w-4 h-4 rounded-full bg-gradient-to-r ${color} node-glow`}
                    />
                  </div>
                  ...
                  <div className="md:hidden ms-16 glass-card rounded-2xl p-6 gradient-border">
                    <h3 className="text-xl font-bold mb-2 font-[family-name:var(--font-space-grotesk)]">{exp.role}</h3>
                    <p className="text-[#3e7bfa] font-medium mb-1">{exp.company}</p>
                    <p className="text-[#5b6478] text-sm mb-2">{exp.location}</p>
                    <p className="text-[#5b6478] text-sm mb-4">{exp.description}</p>
                    <div className="flex flex-col gap-2">
                      {exp.achievements.map((a) => (
                        <div key={a} className="flex items-start gap-2">
                          <ChevronRight size={16} className="text-[#3e7bfa] mt-0.5 flex-shrink-0 rtl:rotate-180" />
                          <span className="text-sm text-[#93a0b8]">{a}</span>
                        </div>
                      ))}
                    </div>
                  </div>
